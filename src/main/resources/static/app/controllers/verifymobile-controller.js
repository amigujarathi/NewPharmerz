myApp.controller('VerifymobileCtrl', function($scope,$rootScope,$route,$location,$cookies,Page,errorMessage,$http,sessionmaintain,sessionout,ContextPath,$timeout) {
    ...
    Page.setTitle('Verify Mobile'); // To set Page Title using factory
    /*
    ...
    //console.log("userid_Cookie"+userid_Cookie);
    // Retrieving a cookie
    ...
    vm.usermobile = $cookies.get('UserMobile'); // get Cookies for User Mobile
    vm.usermobile = vm.usermobile.replace(/%2B/g , "+");
    ...
    vm.countryCode = '91'; // set default

    vm.OtpField = false; //disabled otp filed

    vm.getOTP = function (verifymobilebox) {

        vm.verifymobile = true;

        vm.mobilenum =  '+' + vm.countryCode + vm.registrationMobile;


        if(verifymobilebox.$valid)
        {
            if(vm.mobilenum == vm.usermobile)
            {
                if(vm.mobileverified == 0)
                {
                    vm.invalidmobilemsg = false;
                    vm.wrongmobilemsg = false;
                    vm.invalidmobilemsg = false;

                    vm.userids = userid_Cookie.replace(/-/g, "").toUpperCase();
                    ...
                    $http.post(ContextPath + '/api/v1/mobile', data, verifymobileconfig)
                        .success(function (data, status, verifymobileconfig) {
                ...
                {
                    vm.wrongmobilemsg = false;
                    vm.invalidmobilemsg = true;
                }
            }
            else
            {
                vm.invalidmobilemsg = false;
                vm.wrongmobilemsg = true;
            }
    ...
    /*
     * Method Name  - verifymobileforms
     * Created By   - Vaibhav Godambe
     * Created On   - 14 March 2017
     * Modified By  -
     * Modified On  -
     * Purpose      - This method is use to verify mobile forms
     */


    vm.verifymobileforms = function (verifymobileotpbox) {

        vm.verifyotp = true;

        if(verifymobileotpbox.$valid)
        {
            vm.sendrequestVerifyMobile = true;

           vm.usersid = userid_Cookie.replace(/-/g , "").toUpperCase();
           ...
           $http.post(ContextPath +'/api/v1/verifymobile', data, verifymobileconfig)
                .success(function (data, status,verifymobileconfig) {
                   //console.log('verifyEmail   '+JSON.stringify(data));
                    vm.sendrequestVerifyMobile = false;
                    vm.MobileVerifySuccess = true;

                    var reloadRoute = function() {
                        vm.MobileVerifySuccess = false;
                        $route.reload();
                    }
