myApp.controller('EnquriesCtrl', function($scope,$rootScope,$route,$location,baseURl,$cookies,errorMessage,Page,$http,DTOptionsBuilder,DTColumnBuilder,sessionmaintain,sessionout,ContextPath) {
    var vm = $scope;
   // alert("Product id : " + $rootScope.productid_sendenq);
    vm.receivedEnq = true; // set default tab open

    //error messages from custom factory method.
    vm.errorMessage=errorMessage;

    Page.setTitle('Enquiry'); // To set Page Title using factory
	/*
    ...
    vm.enquiryidBaseUrl = baseURl + 'enquiries/'; // base url
    ...
    vm.LoadEnquiry = function () {
        vm.loading = true; //show loader

        // Receiver enq Tab
        $http.get(ContextPath +'/api/v1/enquiries/search/findByReceiverOrganizationIdOrderByCreatedDesc?receiverOrganizationId='+ Orgnid_Cookie +'&projection=detail', getenqdtlsconfig)
            .success(function (data, status) {
               //console.log('Received Enquires tbl details  : ' + JSON.stringify(data));
               //console.log('Received Enquires tbl details status : ' + status);
                vm.loading = false; //hide loader
                vm.EnqReceivedtbldata = data._embedded.enquiries; // Received

            })
            .error(function (data, status) {
                vm.loading = false; //hide loader
                sessionout.checksessionout(status);
                //console.log("Received Enquires tbl details Error Data : " +data);
                //console.log("Received Enquires tbl details Error Status : " +status);
            });


        // Sent enq Tab
        $http.get(ContextPath +'/api/v1/enquiries/search/findBySenderOrganizationIdOrderByCreatedDesc?senderOrganizationId='+ Orgnid_Cookie +'&projection=detail', getenqdtlsconfig)
            .success(function (data, status) {
              // console.log('Sent Enquires tbl details  : ' + JSON.stringify(data));
               //console.log('Sent Enquires tbl details status : ' + status);
                vm.loading = false; //hide loader
                vm.EnqSenttbldata = data._embedded.enquiries; // Sent
            ...
            .error(function (data, status) {
                vm.loading = false; //hide loader
                sessionout.checksessionout(status);
                //console.log("Sent Enquires tbl details Error Data : " +data);
                //console.log("Sent Enquires tbl details Error Status : " +status);
            });
    }
    ...
    //vm.message = '';
    vm.ReceivedEnqClickHandler = ReceivedEnqClickHandler;
    vm.SentEnqClickHandler = SentEnqClickHandler;

	$scope.vm.dtOptionsReceivedEnq = DTOptionsBuilder.newOptions()
		// Add Bootstrap compatibility
	    .withBootstrap()
        // Add row Callback
		.withOption('rowCallback', rowCallbackReceivedEnq)
	    .withOption('order', [0, 'asc']);



    $scope.vm.dtOptionsSentEnq = DTOptionsBuilder.newOptions()
    // Add Bootstrap compatibility
        .withBootstrap()
        // Add row Callback
        .withOption('rowCallback', rowCallbackSentEnq)
        .withOption('order', [0, 'asc']);

    function ReceivedEnqClickHandler(user) {
        //console.log(user);
        vm.message =  'ReceivedEnq '+ user[1] + ' - ' + user[4] + ' - ' + user[6];
        //console.log('ReceivedEnqId :'+ user[1]);
        $cookies.put('ReceivedEnqId',user[1]); // set ReceivedEnqId cookies
		$location.path('/receivedenquiry');

    }

    function SentEnqClickHandler(user) {
        //console.log(user);
        vm.message =  'SentEnq '+ user[1] + ' - ' + user[4] + ' - ' + user[6];
        //console.log('SentEnqId :'+ user[1]);
        $cookies.put('SentEnqId',user[1]); // set ReceivedEnqId cookies
        $location.path('/showsendenquiry');
    }

    function rowCallbackReceivedEnq(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        //alert(aData);
        // Unbind first in order to avoid any duplicate handler (see https://github.com/l-lin/angular-datatables/issues/87)
        $('td', nRow).unbind('click');
        $('td', nRow).bind('click', function() {
            $scope.$apply(function() {
                vm.ReceivedEnqClickHandler(aData);
            });
        });
        return nRow;
    }
    ...
    function rowCallbackSentEnq(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        //alert(aData);
        // Unbind first in order to avoid any duplicate handler (see https://github.com/l-lin/angular-datatables/issues/87)
        $('td', nRow).unbind('click');
        $('td', nRow).bind('click', function() {
            $scope.$apply(function() {
                vm.SentEnqClickHandler(aData);
            });
        });
        return nRow;
    }
