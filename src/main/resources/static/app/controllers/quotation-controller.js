myApp.controller('QuotationCtrl', function($scope,$route,$location,baseURl,$cookies,errorMessage,Page,$http,DTOptionsBuilder, DTColumnBuilder,sessionmaintain,sessionout,ContextPath) {
    var vm = $scope;
    vm.receivedQuo = true; // set default tab open

    //error messages from custom factory method.
    vm.errorMessage=errorMessage;

    Page.setTitle('Quotation'); // To set Page Title using factory
	/*
    ...
    vm.quotationidBaseUrl = baseURl + 'quotations/'; // base url
    ...
    vm.Loadquotation = function () {
        vm.loading = true;
        // Sent Quotation Tab

        $http.get(ContextPath +'/api/v1/quotations/search/findBySenderOrganizationIdOrderByCreatedDesc?senderOrganizationId='+ Orgnid_Cookie +'&projection=detail', getquodtlsconfig)
            .success(function (data, status) {
               //console.log('Sent Quotation details  : ' + JSON.stringify(data));
               //console.log('Sent Quotation details status : ' + status);
                vm.loading = false;
                vm.QuoSenttbldata = data._embedded.quotations; //
        ...
        $http.get(ContextPath +'/api/v1/quotations/search/findByReceiverOrganizationIdOrderByCreatedDesc?receiverOrganizationId='+ Orgnid_Cookie +'&projection=detail', getquodtlsconfig)
            .success(function (data, status) {
               //console.log('Received Quotation details  : ' + JSON.stringify(data));
               //console.log('Received Quotation details status : ' + status);
                vm.loading = false;
                vm.QuoReceivedtbldata = data._embedded.quotations; //
    ...
    vm.ReceivedQuoClickHandler = ReceivedQuoClickHandler;
    vm.SentQuoClickHandler = SentQuoClickHandler;

    $scope.vm.dtOptionsReceivedQuo = DTOptionsBuilder.newOptions()
    // Add Bootstrap compatibility
        .withBootstrap()
        // Add row Callback
        .withOption('rowCallback', rowCallbackReceivedQuo)
        .withOption('order', [0, 'asc']);



    $scope.vm.dtOptionsSentQuo = DTOptionsBuilder.newOptions()
    // Add Bootstrap compatibility
        .withBootstrap()
        // Add row Callback
        .withOption('rowCallback', rowCallbackSentQuo)
        .withOption('order', [0, 'asc']);

    function ReceivedQuoClickHandler(user) {
        //console.log(user);
        vm.message =  'ReceivedQuo '+ user[1] + ' - ' + user[4] + ' - ' + user[6];
        //console.log('ReceivedQuoId :'+ user[1]);
        $cookies.put('ReceivedQuoId',user[1]); // set ReceivedEnqId cookies
        $location.path('/receviedquotation');

    }

    function SentQuoClickHandler(user) {
        //console.log(user);
        vm.message =  'SentQuo '+ user[1] + ' - ' + user[4] + ' - ' + user[6];
        //console.log('SentQuoId :'+ user[1]);
        $cookies.put('SentQuoId',user[1]); // set ReceivedEnqId cookies
        $location.path('/sentquotation');
    }

    function rowCallbackReceivedQuo(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        //alert(aData);
        // Unbind first in order to avoid any duplicate handler (see https://github.com/l-lin/angular-datatables/issues/87)
        $('td', nRow).unbind('click');
        $('td', nRow).bind('click', function() {
            $scope.$apply(function() {
                vm.ReceivedQuoClickHandler(aData);
            });
        });
        return nRow;
    }
    ...
    function rowCallbackSentQuo(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        //alert(aData);
        // Unbind first in order to avoid any duplicate handler (see https://github.com/l-lin/angular-datatables/issues/87)
        $('td', nRow).unbind('click');
        $('td', nRow).bind('click', function() {
            $scope.$apply(function() {
                vm.SentQuoClickHandler(aData);
            });
        });
        return nRow;
    }
