myApp.controller('AdminhomeCtrl', function($scope,$route,$location,$cookies,$cookieStore,errorMessage,Page,$http,sessionmaintain,sessionout,ContextPath,$filter,$timeout) {
    ...
    Page.setTitle('Admin Home'); // To set Page Title using factory
    /*
    ...
    console.log('User Details on admin home '+userid_Cookie+' '+vm.fullname);


    if($cookies.get('sendEnq')){

        $route.reload();
        $cookieStore.remove('sendEnq');
    }
 ...
 //console.log("Converted user id : " + vm.verifyUsrId);
    ...
    $http.get( ContextPath +'/api/v1/mobileflagverification/'+ vm.verifyUsrId, mobileflagconfig)
        ...
        $timeout( function(){
            $http.get(ContextPath +'/api/v1/users/'+ userid_Cookie +'/organization', orgndtlsconfig)
                .success(function (data, status) {
                   console.log('Org details  146 : ' + JSON.stringify(data));
                   //console.log('Org details status : ' + status);


                    vm.OrgId = data.id; // orgnization id

                   // vm.OrgId = vm.OrgId.replace(/-/g , "").toUpperCase();
                   ...
                   //console.log('Org Id : ' + vm.OrgId);
                    ...
                    // Sent enq Tab
                    vm.sentEnqLoader = true;


                    $http.get(ContextPath +'/api/v1/enquiries/search/findBySenderOrganizationIdOrderByCreatedDesc?senderOrganizationId='+ vm.OrgId +'&projection=detail', getenqdtlsconfig)
                        .success(function (data, status) {
                           console.log('Sent Enquires details 173 : ' + JSON.stringify(data));
                           //console.log('Sent Enquires details status : ' + status);
                            vm.sentEnqLoader = false;
                            vm.EnqBlockSent = data._embedded.enquiries; // array name
                            vm.SentEnqElement = data.page.totalElements;
                        ...
                        .error(function (data, status) {
                            ...
                            //console.log("Sent Enquires details Error Data : " +data);
                            //console.log("Sent Enquires details Error Status : " +status);
                        });


                    // Receiver enq Tab
                    vm.receivedEnqLoader = true;

                    $http.get(ContextPath +'/api/v1/enquiries/search/findByReceiverOrganizationIdOrderByCreatedDesc?receiverOrganizationId='+ vm.OrgId +'&projection=detail', getenqdtlsconfig)
                        .success(function (data, status) {
                           //console.log('Received Enquires details  : ' + JSON.stringify(data));
                           //console.log('Received Enquires details status : ' + status);
                            vm.receivedEnqLoader = false;
                            vm.EnqBlockReceived = data._embedded.enquiries; // array name
                            vm.ReceivedEnqElement = data.page.totalElements;
                    ...
                    // Sent Quotation Tab
                    vm.sentQuoLoader = true;
                    $http.get(ContextPath +'/api/v1/quotations/search/findBySenderOrganizationIdOrderByCreatedDesc?senderOrganizationId='+ vm.OrgId +'&projection=detail', getenqdtlsconfig)
                        .success(function (data, status) {
                           console.log('Sent Quotation details  : ' + JSON.stringify(data));
                           //console.log('Sent Quotation details status : ' + status);
                            vm.sentQuoLoader = false;
                            vm.QuoBlockSentData = data._embedded.quotations; //
                            vm.SentQuoElement = data.page.totalElements;
                        ...
                        .error(function (data, status) {
                            ...
                            vm.loading = false; //hide loader
                            sessionout.checksessionout(status);
                            //console.log("Sent Quotation details Error Data : " +data);
                            //console.log("Sent Quotation details Error Status : " +status);
                        });




                    // Received Quotation Tab
                    vm.receivedQuoLoader = true;
                    $http.get(ContextPath +'/api/v1/quotations/search/findByReceiverOrganizationIdOrderByCreatedDesc?receiverOrganizationId='+ vm.OrgId +'&projection=detail', getenqdtlsconfig)
                        .success(function (data, status) {
                           //console.log('Received Quotation details  : ' + JSON.stringify(data));
                           //console.log('Received Quotation details status : ' + status);
                            vm.receivedQuoLoader = false;
                            vm.QuoBlockReceivedData = data._embedded.quotations; //
                            vm.ReceivedQuoElement = data.page.totalElements;
                    ...
                    // Sent PO Tab
                    // vm.sentPoLoader = true;
                    // $http.get(ContextPath +'/api/v1/purchaseOrders/search/findBySenderOrganizationIdOrderByCreatedDesc?senderOrganizationId='+ vm.OrgId +'&projection=detail', getenqdtlsconfig)
                    //     .success(function (data, status) {
                   //console.log('Sent PO details  : ' + JSON.stringify(data));
                   //console.log('Sent PO details status : ' + status);
                    //         vm.sentPoLoader = false;
                    //         vm.PoBlockSentData = data._embedded.purchaseOrders; //
                    //         vm.SentPoElement = data.page.totalElements;
                    //
                    //     })
                    //     .error(function (data, status) {
                    //         vm.sentPoLoader = false;
                    //         vm.loading = false; //hide loader
                    //         sessionout.checksessionout(status);
                   //console.log("Sent PO details Error Data : " +data);
                   //console.log("Sent PO details Error Status : " +status);
                    //     });

                    // Received PO Tab
                    // vm.receivedPoLoader = true;
                    // $http.get(ContextPath +'/api/v1/purchaseOrders/search/findByReceiverOrganizationIdOrderByCreatedDesc?receiverOrganizationId='+ vm.OrgId +'&projection=detail', getenqdtlsconfig)
                    //     .success(function (data, status) {
                   //console.log('Received PO details  : ' + JSON.stringify(data));
                   //console.log('Received PO details status : ' + status);
                    //         vm.receivedPoLoader = false;
                    //         vm.PoBlockReceivedData = data._embedded.purchaseOrders; //
                    //         vm.ReceivedPoElement = data.page.totalElements;
                    //
                    //     })
                    //     .error(function (data, status) {
                    //         vm.receivedPoLoader = false;
                    //         vm.loading = false; //hide loader
                    //         sessionout.checksessionout(status);
                   //console.log("Received PO details Error Data : " +data);
                   //console.log("Received PO details Error Status : " +status);
                    //     });
    ...
    vm.matchPassword = function(){
        if(vm.password == vm.cpassword){
            vm.showerror = false;
            //console.log("showeerror false");
        }else{
            vm.showerror = true;
            //console.log("showeerror true");
        }
    }//close matchPassword
