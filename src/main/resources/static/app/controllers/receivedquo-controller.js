myApp.controller('ReceivedQuoCtrl', function($scope,$rootScope,$route,baseURl,$location,$cookies,errorMessage,Page,$http,DTOptionsBuilder, DTColumnBuilder,sessionmaintain,sessionout,ContextPath,$filter,Upload) {
    var vm = $scope;
    ...
    Page.setTitle('Received Quotation'); // To set Page Title using factory
    /*
    ...
    // Receiver enq Tab
    $http.get(ContextPath +'/api/v1/quotations/'+ ReceivedQuoId_Cookie +'?projection=detail' , getquodtlsconfig)
        .success(function (data, status) {
           //console.log('Received Quotation details  : ' + JSON.stringify(data));
           //console.log('Received Quotation details status : ' + status);

            vm.QuoReceiveddata = data; // sent
            ...
            vm.PaymentsTerms = data.paymentTerms;
            vm.paymentTermsOther = data.paymentTermsOther;

            if(vm.PaymentsTerms == "OTHER")
            {
                vm.PaymentsTerms = vm.paymentTermsOther;
            }


            vm.PaymentsTerm = $filter('Camlecase')(vm.PaymentsTerm);
            vm.leadtime = data.leadTime;
            vm.offervalidity = data.offerValidity;
            vm.msg = data.message;
            vm.frieghttype = data.frieghtType;
            vm.packaging = data.packaging;
            vm.receivedQuoShipmentBy = data.shipmentBy;
            //console.log("Received quoattion shipmentby : " +vm.receivedQuoShipmentBy);
            //console.log("Received quoattion typeof shipmentby" + typeof vm.receivedQuoShipmentBy);
            ...
            $http.get(ContextPath +'/api/v1/organizations/'+ vm.senderorgId +'/user' , getquodtlsconfig)
                .success(function (data, status) {
    ...
    /*
     * method Name  - GenratePurchaseorder
     * Created By   - vaibhav godambe
     * Created On   - 1 feb 2017
     * Modified By  -
     * Modified On  -
     * Purpose      - This method is used to move to GenratePurchaseorder screen.
     */

    vm.GenratePurchaseorder = function(receivedquotation){
        vm.receivedQuo = true;
        if(receivedquotation.$valid){
            $location.path('/generatepo'); //  move to the GenratePurchaseorder screen
        }
    }

    /*
     * method Name  - changeQuotationStatus
     * Created By   - vaibhav godambe
    ...
    vm.changeQuotationStatus = function () {

        vm.sendrequestUpdateQuotationStatus = true;

        var data = {
            status : 'ACCEPTED',
        };

        //console.log('Quotation status : ' + JSON.stringify(data));

        //console.log('Quotation id: ' + vm.receivedQuotationId);

        $http.patch(ContextPath +'/api/v1/quotations/'+vm.receivedQuotationId, data ,updatestatusconfig)
            .success(function (data, status) {
                //console.log('Quotation status update details  : ' + JSON.stringify(data));
                //console.log('Quotation status update status : ' + status);
                vm.sendrequestUpdateQuotationStatus = false;
    ...
    vm.channel = ReceivedQuoId_Cookie + 'quo' ; // Unique channel Id
