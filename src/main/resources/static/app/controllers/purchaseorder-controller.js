myApp.controller('PurchaseorderCtrl', function($scope,$route,$location,baseURl,$cookies,errorMessage,Page,$http,DTOptionsBuilder, DTColumnBuilder,sessionmaintain,sessionout,ContextPath) {
    var vm = $scope;
    vm.receivedPO = true; // set default tab open

    //error messages from custom factory method.
    vm.errorMessage=errorMessage;

    Page.setTitle('Purchase Order'); // To set Page Title using factory
	/*
    ...
    vm.POidBaseUrl = baseURl + 'purchaseOrders/'; // base url
    ...
    vm.LoadPrurchseorder = function () {

        vm.loading = true;
    // Sent PO Tab

    $http.get(ContextPath +'/api/v1/purchaseOrders/search/findBySenderOrganizationIdOrderByCreatedDesc?senderOrganizationId='+ Orgnid_Cookie +'&projection=detail', getpodtlsconfig)
        .success(function (data, status) {
           //console.log('Sent PO details  : ' + JSON.stringify(data));
           //console.log('Sent PO details status : ' + status);
            vm.loading = false;
            vm.PoSenttbldata = data._embedded.purchaseOrders; //
    ...
    $http.get(ContextPath +'/api/v1/purchaseOrders/search/findByReceiverOrganizationIdOrderByCreatedDesc?receiverOrganizationId='+ Orgnid_Cookie +'&projection=detail', getpodtlsconfig)
        .success(function (data, status) {
           //console.log('Received PO details  : ' + JSON.stringify(data));
           //console.log('Received PO details status : ' + status);
            vm.loading = false;
            vm.PoReceivedtbldata = data._embedded.purchaseOrders; //
    ...
    vm.ReceivedPoClickHandler = ReceivedPoClickHandler;
    vm.SentPoClickHandler = SentPoClickHandler;

    $scope.vm.dtOptionsReceivedPo = DTOptionsBuilder.newOptions()
    // Add Bootstrap compatibility
        .withBootstrap()
        // Add row Callback
        .withOption('rowCallback', rowCallbackReceivedPo)
        .withOption('order', [0, 'asc']);



    $scope.vm.dtOptionsSentPo = DTOptionsBuilder.newOptions()
    // Add Bootstrap compatibility
        .withBootstrap()
        // Add row Callback
        .withOption('rowCallback', rowCallbackSentPo)
        .withOption('order', [0, 'asc']);

    function ReceivedPoClickHandler(user) {
        //console.log(user);
        vm.message =  'ReceivedPo '+ user[1] + ' - ' + user[4] + ' - ' + user[6];
        //console.log('ReceivedPoId :'+ user[1]);
        $cookies.put('ReceivedPoId',user[1]); // set ReceivedEnqId cookies
        $location.path('/receivedpo');

    }

    function SentPoClickHandler(user) {
        //console.log(user);
        vm.message =  'SentPo '+ user[1] + ' - ' + user[4] + ' - ' + user[6];
        //console.log('SentPoId :'+ user[1]);
        $cookies.put('SentPoId',user[1]); // set ReceivedEnqId cookies
        $location.path('/sentpo');
    }

    function rowCallbackReceivedPo(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        //alert(aData);
        // Unbind first in order to avoid any duplicate handler (see https://github.com/l-lin/angular-datatables/issues/87)
        $('td', nRow).unbind('click');
        $('td', nRow).bind('click', function() {
            $scope.$apply(function() {
                vm.ReceivedPoClickHandler(aData);
            });
        });
        return nRow;
    }
    ...
    function rowCallbackSentPo(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        //alert(aData);
        // Unbind first in order to avoid any duplicate handler (see https://github.com/l-lin/angular-datatables/issues/87)
        $('td', nRow).unbind('click');
        $('td', nRow).bind('click', function() {
            $scope.$apply(function() {
                vm.SentPoClickHandler(aData);
            });
        });
        return nRow;
    }
