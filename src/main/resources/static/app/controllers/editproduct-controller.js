myApp.controller('EditproductCtrl', function($scope,$rootScope,$route,$location,$cookies,Page,baseURl,errorMessage,$http,sessionmaintain,sessionout,getcategorydetails,ContextPath) {
    ...
    Page.setTitle('Edit Product'); // To set Page Title using factory
    /*
    ...
    // Retrieving a cookie
    var productId_cookie = $cookies.get("productId"); //get product Id cookies
    //console.log("productId_cookie : " +productId_cookie);
    ...
    /*
     * Api Name  - get edit product data
     * Created By   - vaibhav godambe
     * Created On   - 6 feb 2017
     * Modified By  -
     * Modified On  -
     * Purpose      - This factory is used to get edit product data.
     */

    $http.get(ContextPath +'/api/v1/products/'+productId_cookie +'?projection=detail' , geteditproductconfig)
        .success(function (data, status, geteditproductconfig) {
            //console.log("Edit product Data : " +JSON.stringify(data));
            //console.log("Edit product Status : " +status);

                // set to all variables
            vm.productname = data.product;
            vm.composition = data.composition;
            vm.productcategory = data.categoryName;
            //vm.sku =
            vm.upc = data.upc;
            //vm.productTodo =
            vm.notes = data.note;
    ...
    /*
     * Medthod Name  - editproductdata
     * Created By   - vaibhav godambe
     * Created On   - 6 Feb 2017
     * Modified By  -
     * Modified On  -
     * Purpose      - This Medthod is used to update product data.
     */

    //config parameter is need to send client data to server in ajax request
    var editproductconfig = {
        headers : {
            'Accept' : 'application/json',
            'Content-Type': 'application/json',
        }
    ...
    vm.editproductdata = function (editproductform) {
        vm.editproduct =  true;
        ...
        $http.get(ContextPath +'/api/v1/users/'+userid_Cookie, getuserdetailsconfig)
            .success(function (data, status) {
              ...
              vm.adminName = vm.fName + ' ' + vm.lName;
                ...
                // error parametere set by default value
                // vm.showerror = false; // by default value false
