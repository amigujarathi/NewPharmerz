myApp.controller('TaxCtrl', function($scope,$route,$rootScope,$location,$cookies,$timeout,baseURl,Page,$http,errorMessage,sessionmaintain,sessionout,ContextPath) {
    ...
    Page.setTitle('Taxes'); // To set Page Title using factory
    /*
    ...
    /*
     * Api Name  - Get All Taxes of Orgnization
     * Created By   - vaibhav godambe
     * Created On   - 7 feb 2017
     * Modified By  -
     * Modified On  -
     * Purpose      - This api is used to get all taxes using orgnization id.
     */

    $http.get(ContextPath +'/api/v1/taxes/search/findByOrganizationId?organizationId='+Orgnid_Cookie,taxconfig)
        .success(function(data, status, taxconfig){
            //console.log("OrgnizationTax Data : " +JSON.stringify(data));
            //console.log("OrgnizationTax Status : " +status);

            vm.taxdata = data._embedded.taxes; // tax data

            vm.elementCount = data.page.totalElements; // page count

        }).error(function (data, status) {
            sessionout.checksessionout(status);
            //console.log("OrgnizationTax Error Data : " +data);
            //console.log("OrgnizationTax Error Status : " +status);
        });
    ...
    /*
     * Method Name  - resetEditTaxFormData
     * Created By   - vaibhav godambe
     * Created On   - 9 March 2017
     * Modified By  -
     * Modified On  -
    ...
    vm.resetEditTaxFormData = function () {
        vm.edittax = false; // To hide the error
    ...
    vm.deleteTaxes = function (link) {
        //console.log(" tax link : " + link);
            vm.taxlink = link; // set parameter
        angular.element('#DeletetaxModal').modal('toggle'); // open modal window

    }

    /*
     * Method Name  - deleteTax
     * Created By   - vaibhav godambe
     * Created On   - 7 feb 2017
     * Modified By  -
     * Modified On  -
     * Purpose      - This method is used to delete taxes.
     */

    vm.deleteTax = function () {

        //console.log(vm.taxlink);
        vm.sendrequest = true; //  show loader
        $http.delete(vm.taxlink, taxconfig)
            .success(function (data, status) {
                //console.log('Tax delete details  : ' + JSON.stringify(data));
                //console.log('Tax delete details status : ' + status);

                angular.element('.modal-backdrop').remove();
                angular.element('body').removeClass('modal-open');
                angular.element('body').removeAttr('style');
                angular.element('#DeletetaxModal').css('display','none');
                vm.sendrequest = false; //  hide loader
                $route.reload();
            })
            .error(function (data, status) {
                vm.sendrequest = false; //  hide loader
                sessionout.checksessionout(status);
                //console.log("Tax delete details Error Data : " +data);
                //console.log("Tax delete details Error Status : " +status);
            });
    }

    /*
     * Method Name  - getEditTaxData
     * Created By   - vaibhav godambe
     * Created On   - 7 feb 2017
     * Modified By  -
     * Modified On  -
     * Purpose      - This method is used to get edit taxes data.
     */

    vm.getEditTaxData = function (taxid) {
        //console.log('TaxId : ' +taxid );

        vm.EditTaxId = taxid; // assign tax id

        $http.get(ContextPath +'/api/v1/taxes/'+taxid,taxconfig)
            .success(function(data, status, taxconfig){
                //console.log("Get Edit Tax Data : " +JSON.stringify(data));
                //console.log("Get Edit Tax Data Status : " +status);

                vm.edittaxname = data.tax;
                vm.editpercent = data.percent;
                vm.editdescription = data.description;


            }).error(function (data, status) {
            sessionout.checksessionout(status);
            //console.log("Get Edit Tax Data Error Data : " +data);
            //console.log("Get Edit Tax Data Error Status : " +status);
        });
    }

    /*
     * Method Name  - edittaxes
     * Created By   - vaibhav godambe
     * Created On   - 7 feb 2017
     * Modified By  -
     * Modified On  -
     * Purpose      - This method is used to edit taxes data.
     */

    vm.edittaxes = function (edittaxform) {
        vm.edittax = true;
        ...
        if(edittaxform.$valid)
        {
            ...
            var data = {
                organization : vm.OrgBaseurl,
                tax: vm.edittaxname,
                percent : vm.editpercent,
                description : vm.editdescription,
            };

            //console.log('Data : '  + JSON.stringify(data));

            $http.patch(ContextPath +'/api/v1/taxes/'+vm.EditTaxId,data,taxconfig)
                .success(function(data, status, taxconfig){
                    //console.log("Edit OrgnizationTax Data : " +JSON.stringify(data));
                    //console.log("Edit OrgnizationTax Status : " +status);

                    angular.element('.modal-backdrop').remove();
                    angular.element('body').removeClass('modal-open');
                    angular.element('body').removeAttr('style');
                    angular.element('#EdittaxModal').css('display','none');
                ...
                vm.sendrequest = false; //  hide loader
                sessionout.checksessionout(status);
                //console.log("Edit OrgnizationTax Error Data : " +data);
                //console.log("Edit OrgnizationTax Error Status : " +status);
            });
